// Рекламные аккаунты компании. Единственный источник правды: карточка
// кампании, фильтр KPI и модель SMM должны писать и читать одно и то же
// значение. Раньше форма кампании предлагала «Anuar», а всё остальное
// работало с «ANUAR» — в фильтре появлялся третий, несуществующий аккаунт.
export const CAMPAIGN_ACCOUNTS = ['FRANCHONE', 'ANUAR'] as const
export type CampaignAccount = (typeof CAMPAIGN_ACCOUNTS)[number]

// Приводит написание аккаунта к каноническому. Нужен для старых записей:
// в базе встречаются «Anuar» и «anuar».
export function normalizeAccount(raw: string): string {
  const t = raw.trim()
  const hit = CAMPAIGN_ACCOUNTS.find((a) => a.toLowerCase() === t.toLowerCase())
  return hit ?? t
}
