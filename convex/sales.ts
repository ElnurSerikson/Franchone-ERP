...
// Дата вступления в силу §2 дополнения: с этого дня единственный источник
// количества заявок — отчёт таргетолога. За более ранние дни воронка читает
// то, что менеджеры вводили руками до дополнения, иначе вся прошлая история
// разом обнулилась бы и отчёты выглядели бы сломанными.
//
// Отсечка нужна именно как гарантия: новых ручных значений не появляется —
// поле убрано из мутации, — но без явной границы старое число могло бы
// всплыть в новом периоде при правке давнего отчёта администратором.
const LEADS_HANDOVER_DATE = '2026-08-08'
...
export async function salesSummary(
...
) {
  {
    ...
    // Переходный период: за дни ДО вступления правила в силу показываем то,
    // что менеджеры вводили руками. Это не суммирование — запасное значение
    // берётся только при полном отсутствии записи таргетолога. С даты
    // передачи показателя запаса нет: нет его отчёта — значит ноль, и это
    // видно сразу, а не прячется за старой цифрой.
    const legacyLeads = new Map<string, number>()
    for (const r of reports) {
      if (r.newLeads > 0 && r.date < LEADS_HANDOVER_DATE) {
        legacyLeads.set(`${r.objectId}|${r.date}`, r.newLeads)
      }
    }
